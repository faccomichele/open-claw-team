...
This folder contains the **source-of-truth definitions** for all GitHub Copilot custom agents used across project repos. Each `.md` file defines an agent's role, expertise, constraints, and quality checklist.
...
When an agent definition changes here, copy the updated file to `.github/copilot/agents/` in every project repo that uses that agent.
...
Copy each MD definition from `github_copilot_agents/` to `.github/copilot/agents/` in the target project repo.
